...
extension GlucoseValue {
    ...
    public func decayEffect(atRate rate: LoopQuantity, for duration: TimeInterval, withDelta delta: TimeInterval = 5 * 60, useLegacyDecay: Bool = false) -> [GlucoseEffect] {
        ...
        // Pre-#33 (deployed Loop): accumulate the decay step-by-step from the floored
        // simulation boundary. For samples not aligned to delta boundaries (real,
        // jittery CGM timestamps) this exhibits a small discontinuity at bucket
        // boundaries; #33 replaced it with the closed-form quadratic below, which is
        // identical for delta-aligned samples.
        if useLegacyDecay {
